...
int main(int argc, char * argv[])
{
  ...
  while (1) {
    ...
    if (ch == 'v') {
      std::cout << "paircomp toolkit version " << VERSION \
		<< "; find_patch binary v" << BIN_VERSION << "\n";
      do_exit = true;
    }
  ...
  if (optind != argc - 5) {
    std::cerr << "Usage:\n\t" << argv[0] \
	      << " seq1 seq2 windowsize threshold out\n\n";
    exit(-1);
  }
  ...
  threshold = atof(argv[optind + 3]);
  file_out = argv[optind + 4];

  // Verify that windowsize & threshold are reasonable.
  if (windowsize < 5 || windowsize > 12) {
    std::cerr << "ERROR: windowsize must be 5 < w < 12.\n";
    exit(-1);
  }
  ...
  std::cerr << "Doing a paircomp with windowsize " << windowsize \
	    << " and threshold " << threshold << ".\n";

  // Do paircomp.
  paircomp::ImmutableComparison * cmp;
  cmp = paircomp::hashed_n_comparison(seq1, seq2, windowsize, threshold);

  // Write to output file.
  cmp->save_as_paircomp(file_out);
...
std::string read_fasta_seq(char * filename)
{
  ...
  std::cerr << "Loaded sequence '" << name << "' from file '" \
